...
export function AppMigration() {
            ...
            try {
              ...
              switch (event.type) {
                ...
                case 'log':
                  ...
                  break;
                case 'thought':
                  if (event.content) setThoughts(prev => [...prev, event.content!]);
                  break;
                case 'result':
                  if (event.report) {
  ...
  return (
    <div className="min-h-screen w-full bg-zinc-950 p-6 flex flex-col font-sans text-zinc-300">
      {/* Header */}
      ...
      {migrationState === 'streaming' && (
        <div className="flex-1 flex flex-col md:flex-row gap-6">
          ...
          <div className="flex-1 flex flex-col gap-6 w-full md:w-2/3 h-[70vh]">
            <div className="flex-1 bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 overflow-y-auto flex flex-col" ref={thoughtRef}>
      ...
      {(migrationState === 'complete' || migrationState === 'error') && (
        <div className={`max-w-5xl mx-auto w-full bg-zinc-900/50 border ${migrationState === 'error' ? 'border-red-500/30' : 'border-emerald-500/30'} rounded-2xl p-8 flex flex-col items-center mt-10`}>
          {migrationState === 'error' ? (
            ...
            <>
              <h3 className="text-3xl font-bold text-zinc-100 mb-4 text-center">Migration Complete!</h3>
              <p className="text-zinc-400 mb-8 max-w-2xl text-center">The application was successfully refactored to use Google Cloud Spanner configurations.</p>

              <div className="w-full bg-zinc-950 rounded-xl p-6 border border-zinc-800 mb-8 text-zinc-300 text-sm overflow-x-auto font-mono whitespace-pre-wrap">
                {resultReport}
              </div>
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  className="bg-emerald-600 hover:bg-emerald-500 text-white font-medium px-8 py-3 rounded-xl shadow-lg shadow-emerald-500/20 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <FolderSync size={18} />
